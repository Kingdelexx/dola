  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        ...
        const s2Progress = user.profile?.stage2_progress ?? (localStorage.getItem('stage2_progress') ? parseInt(localStorage.getItem('stage2_progress') || '0', 10) : 0);
        const s3Completed = localStorage.getItem('stage3_completed') === 'true';
        const isAdminOrTeacher = user.profile?.role && ['teacher', 'school_admin', 'super_admin'].includes(user.profile.role);
        if ((s1Progress < 80 || s2Progress < 11 || !s3Completed) && !isAdminOrTeacher) {
