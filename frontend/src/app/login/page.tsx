  ...
  useEffect(() => {
    ...
    const initGoogleBtn = () => {
      if (typeof window !== 'undefined' && (window as unknown as { google?: { accounts?: { id?: { initialize: (config: object) => void; renderButton: (el: HTMLElement, config: object) => void; prompt: (notification?: any) => void } } } }).google?.accounts?.id) {
        ...
        const btnContainer = document.getElementById('googleLoginBtnDiv');
        if (btnContainer) {
      ...
      <div className="max-w-5xl w-full bg-white border-4 border-white rounded-[2.5rem] shadow-[0_20px_50px_-10px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col md:flex-row-reverse relative z-10">
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 lg:p-16 bg-slate-50 relative">
          <div ref={titleRef} className="mb-8 text-center md:text-left">
            ...
            <p className="text-slate-500 font-medium text-base">Sign in with Gmail (no password) or email.</p>
          </div>

          {/* Google Sign In Button */}
          <div className="mb-6 flex flex-col gap-3">
            <div id="googleLoginBtnDiv" className="w-full flex justify-center min-h-[44px]"></div>
            <button
              type="button"
              disabled={isLoading}
              onClick={() => setShowGmailModal(true)}
              className="w-full py-2.5 px-4 bg-gradient-to-r from-sky-50 to-indigo-50 hover:from-sky-100 hover:to-indigo-100 border-2 border-sky-200 text-sky-700 font-extrabold rounded-2xl transition-all flex items-center justify-center gap-2 cursor-pointer text-xs shadow-xs"
            >
